...
func (r *repository) GetAllDrafts(insuranceCode string) ([]template.TemplateDraft, error) {
	...
	if insuranceCode != "" {
		query = `
			...
			FROM property_service_development.template_drafts
...
func (r *repository) UpdateDraft(id string, draft *template.TemplateDraft) error {
	...
	query := `
		UPDATE property_service_development.template_drafts
...
func (r *repository) DeleteDraft(id string) error {
	query := `DELETE FROM property_service_development.template_drafts WHERE id = ?`
...
func (r *repository) ClearDrafts() error {
	query := `DELETE FROM property_service_development.template_drafts`
...
func (r *repository) GetByID(locale, id string) (*template.Template, error) {
	query := `
		...
		FROM property_service_development.templates
		WHERE locale = ? AND id = ?
	`
...
func (r *repository) Create(t *template.Template) error {
	query := `
		INSERT INTO property_service_development.templates (locale, id, value, created_by, created_at)
		VALUES (?, ?, ?, ?, NOW())
		ON DUPLICATE KEY UPDATE
			value = VALUES(value),
			created_by = VALUES(created_by)
...
func (r *repository) Update(locale, id string, t *template.Template) error {
	...
	query := fmt.Sprintf(`
		UPDATE property_service_development.templates
		SET %s
		WHERE locale = ? AND id = ?
...
func (r *repository) Delete(locale, id string) error {
	query := `DELETE FROM property_service_development.templates WHERE locale = ? AND id = ?`
...
func (r *repository) GetTemplateBeforeUpdate(locale, id string) (*template.TemplateBeforeUpdate, error) {
	query := `SELECT locale, id, value FROM property_service_development.templates WHERE locale = ? AND id = ?`
