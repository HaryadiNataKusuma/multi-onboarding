...
func (u *usecase) Create(req product.CreateProductRequest) error {
	...
	p := &product.Product{
		...
		SchengenEligible: req.SchengenEligible,
		MinAdult:        req.MinAdult,
		MaxAdult:        req.MaxAdult,
		MinChild:        req.MinChild,
		MaxChild:        req.MaxChild,
		Summary:         req.Summary,
...
func (u *usecase) Update(id int, req product.UpdateProductRequest) error {
	...
	existing.SchengenEligible = req.SchengenEligible
	existing.MinAdult = req.MinAdult
	existing.MaxAdult = req.MaxAdult
	existing.MinChild = req.MinChild
	existing.MaxChild = req.MaxChild
...
func (u *usecase) GenerateTemplateDraftsForAllProducts(createdBy int64) (int, error) {
	return u.repository.GenerateTemplateDraftsForAllProducts(createdBy)
}
