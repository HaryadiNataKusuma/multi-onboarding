...
// AddDraft handles POST /api/insurances/draft/add
...
// GetAllDrafts handles GET /api/insurances/draft
...
// GetDraftByID handles GET /api/insurances/draft/:id
...
// UpdateDraft handles PUT /api/insurances/draft/:id
...
// DeleteDraft handles DELETE /api/insurances/draft/:id
...
// ClearDrafts handles POST /api/insurances/draft/clear
...
// ConfirmDrafts handles POST /api/insurances/draft/confirm
...
// GetAll handles GET /api/insurances
func (h *handlerInsurance) GetAll(c echo.Context) error {
	c.Logger().Info("GetAll insurances: Request received")
	
	insurances, err := h.usecase.GetAll()
	if err != nil {
		c.Logger().Errorf("GetAll insurances error: %v", err)
		return c.JSON(http.StatusInternalServerError, map[string]string{
			"error": err.Error(),
		})
	}
	...
	c.Logger().Infof("GetAll insurances: Returning %d insurances", len(insurances))
	if len(insurances) > 0 {
		c.Logger().Infof("GetAll insurances: First insurance - Code: %s, Name: %s", insurances[0].Code, insurances[0].Name)
	}
	
	return c.JSON(http.StatusOK, insurances)
}

// GetByID handles GET /api/insurances/:id
...
// Create handles POST /api/insurances
...
// Update handles PUT /api/insurances/:id
...
// Delete handles DELETE /api/insurances/:id
