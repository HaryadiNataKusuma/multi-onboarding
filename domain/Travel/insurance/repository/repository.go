...
func (r *repository) Create(ins *insurance.Insurance) error {
	...
	query := `
		INSERT INTO insurances (code, name, is_active, logo, created_at, updated_at)
		VALUES (?, ?, ?, ?, NOW(), NOW())
	`
...
func (r *repository) GetAll() ([]insurance.Insurance, error) {
	...
	// Log for debugging
	if count == 0 {
		fmt.Printf("⚠️  GetAll insurances: No data found in database\n")
	} else {
		fmt.Printf("✅ GetAll insurances: Found %d insurances\n", count)
	}
...
func (r *repository) Update(id int, ins *insurance.Insurance) error {
	...
	// Build update query (using is_active column)
	...
	if len(changedBefore) > 0 {
		...
		_ = r.InsertHistory("system", "insurances", "UPDATE", id, "insurance", string(beforeJSON), string(afterJSON))
	}
...
func (r *repository) GetInsuranceBeforeUpdate(id int) (*insurance.InsuranceBeforeUpdate, error) {
	// Use is_active column (table uses is_active, not status)
	query := `SELECT code, name, is_active, logo FROM insurances WHERE id = ?`
