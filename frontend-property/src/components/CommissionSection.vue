...
export default {
  ...
  data() {
    return {
      ...
      COMMISSIONS_API_URL: 'http://localhost:8080/api/property/commissions', // Property-specific route
      // Filter states
      filterCommissions: {
        product_code: '',
        agent_level: ''
  ...
  methods: {
    ...
    async saveCommission() {
      ...
      try {
        ...
        for (const product of filteredProducts) {
          // Get hardcopy_admin_fee from product (from hardcopy_admin_fee column in products table)
          const hardcopyFee = product.hardcopy_admin_fee != null ? parseInt(product.hardcopy_admin_fee, 10) : (product.admin_fee != null ? parseInt(product.admin_fee, 10) : 0)
    ...
    async startEditFromResult(item, tableType) {
      // For plan_commissions, fetch hardcopy_fee from product if not available
      let hardcopyFee = item.hardcopy_fee || 0
      if (tableType === 'plan_commissions' && (!item.hardcopy_fee || item.hardcopy_fee === 0)) {
        // Try to get hardcopy_admin_fee from product
        try {
          const productCode = item.plan_code || item.product_code
          if (productCode) {
            const productsResponse = await fetch(`${this.API_URL}/products`)
            if (productsResponse.ok) {
              ...
              if (product) {
                hardcopyFee = product.hardcopy_admin_fee != null ? parseInt(product.hardcopy_admin_fee, 10) : (product.admin_fee != null ? parseInt(product.admin_fee, 10) : 0)
              }
            }
          }
        } catch (error) {
          console.error('Error fetching product for hardcopy_fee:', error)
        }
      }
