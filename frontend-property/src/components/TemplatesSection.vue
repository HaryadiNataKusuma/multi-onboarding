...
export default {
  ...
  computed: {
    ...
    isInsuranceDetailCar() {
      return this.editForm.id && 
             (this.editForm.id.includes('INSURANCE_DETAIL_TV') || this.editForm.id.includes('INSURANCE_DETAIL_PR'))
    },
    isAddFormInsuranceDetailCar() {
      return this.form.id && 
             (this.form.id.includes('INSURANCE_DETAIL_TV') || this.form.id.includes('INSURANCE_DETAIL_PR'))
    },
    isBulkEditInsuranceDetail() {
      ...
      // Check if all selected items are INSURANCE_DETAIL_
      return itemsToCheck.every(item => {
        const templateId = item.item?.id || item.id
        return templateId && (templateId.includes('INSURANCE_DETAIL_TV') || templateId.includes('INSURANCE_DETAIL_PR'))
      })
    },
  ...
  methods: {
    ...
    openBulkEdit(isDraft) {
      ...
      // Parse HTML from first selected item if it's INSURANCE_DETAIL_
      this.$nextTick(() => {
